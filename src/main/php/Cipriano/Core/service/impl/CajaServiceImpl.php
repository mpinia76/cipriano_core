...
use Cipriano\Core\service\ICajaService;
...
class CajaServiceImpl extends CrudService implements ICajaService {
	...
	function abrirCaja(Caja $caja, User $user ){
		...
		$transferencia->setOrigen( CiprianoUtils::getCuentaCajaChica() );
	...
	function cerrarCaja(Caja $caja, User $user){
		...
		try {
			...
			$transferencia->setDestino( CiprianoUtils::getCuentaCajaChica() );
