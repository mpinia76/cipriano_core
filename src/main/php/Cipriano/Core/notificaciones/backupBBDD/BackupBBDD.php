<?php
namespace Cipriano\Core\notificaciones\backupBBDD;

use Cipriano\Core\utils\XTemplate;
...
class BackupBBDD{
	...
	function backupDatabaseTables($dbHost,$dbUsername,$dbPassword,$dbName, $filename,$tables = '*' ){
	    ...
	    //save file
		$nameFile ='db_cipriano_' . date("Y-m-d-H-i-s") .'.sql';
	...
	public function send(){
		...
		$this->setPathBackup(CiprianoConfig::getInstance()->getAppPathCore()."/backup/");

		$filename = 'db_cipriano_' . date("Y-m-d-H-i-s") . '.zip';
		$backupFile = $this->getPathBackup().$filename;

		self::backupDatabaseTables(CiprianoConfig::getInstance()->getDbHost(),CiprianoConfig::getInstance()->getDbUser(),CiprianoConfig::getInstance()->getDbPassword(),CiprianoConfig::getInstance()->getDbName(),$filename);
		...
		$subjectMail = 'BackUp cose_cipriano';
		...
		$template->assign("titulo", "En el archivo adjunto se encuentra el BackUp de la BBDD cose_cipriano realizado el ".date('d/m/Y H:i:s'));
		...
		CiprianoUtils::sendMail(CiprianoConfig::TEST_MAIL_TO,CiprianoConfig::TEST_MAIL_TO, $subjectMail, $mensaje, $attachs);
		unlink($backupFile);
